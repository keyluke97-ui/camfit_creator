...
Task 3~7의 **코드는 필드가 없어도 작성·타입검사까지 가능**하다. 하지만 **Task 14(브라우저 검증)는 필드가 실제로 있어야** 한다 (Airtable은 없는 필드에 쓰면 422로 실패한다).

Task 1에서 회신 문서를 쓰고, Task 2에서 필드를 만든다. 캠지기측 회신이 늦어지면 Task 3~13을 먼저 진행해도 된다.
...
## Task 2: Airtable 신규 필드 7개 생성

**선행:** Task 1 회신에 대한 캠지기측 동의

**Base** `appEGM6qarNr9M7HN` / **Table** `크리에이터 명단` `tblkuPln7nquA3dLA`

- [ ] **Step 1: 필드 7개 생성 (Airtable MCP `create_field`)**

| 필드명 | 타입 | 옵션 |
|---|---|---|
| `유튜브 채널 강점` | multilineText | — |
| `인스타 채널 강점` | multilineText | — |
| `블로그 채널 강점` | multilineText | — |
| `프로필 심사 상태` | singleSelect | `심사대기` `승인` `반려` |
| `지표 재검토 필요` | checkbox | — |
| `심사 반려 사유` | multilineText | — |
| `심사 처리 일시` | dateTime | — |

각 필드에 description을 넣는다 (기존 지명형 필드들이 전부 description을 갖고 있다). 예: `프로필 심사 상태` → *"자기신고 지표 검증. 승인 전에는 캠지기에게 노출 금지. 1a-v2 §4"*

- [ ] **Step 2: 실측 확인**

Airtable MCP `describe_table`로 `tblkuPln7nquA3dLA`를 다시 읽어 **필드명 7개가 정확히 일치**하는지 확인한다.

> ⚠️ 이 확인을 건너뛰지 마라. 2026-07-16에 필드명을 가정으로 코딩했다가 read는 조용히 `undefined`, write는 422로 실패할 뻔했다.

- [ ] **Step 3: 커밋 없음** (Airtable은 git 밖)
