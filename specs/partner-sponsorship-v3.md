...
| `총 팔로워 쿠폰 수` | Formula | `{등급별 모집 희망 인원 합계} × {인당 팔로워 쿠폰}` (v3.1 정의) |

#### 🆕 v3.1 신규 — 어드민 자동 발행 + 동기 분배 (2026-04-28~)

| 필드명 | 타입 | 설명 |
|------|------|------|
| `🎟️ 쿠폰 자동 발행` | Checkbox | 운영자 ON → 어드민 자동화 10초 내외로 풀 채움 후 자동 해제 |
| `팔로워 쿠폰 코드` | Long text | 어드민 자동 발행이 채우는 N줄 풀. 신청 시 첫 줄을 슬라이싱해 분배. |
| `배포 완료된 쿠폰` | Long text | 신청자에게 분배된 코드 이력 누적 |
| `🎟️ 발행 상태` | Single select | `대기` / `전체성공` / `일부성공` / `실패` (운영팀 전용, 포털 비노출) |
| `🎟️ 마지막 에러 메시지` | Long text | 발행 실패 시 디버깅 (운영팀 전용, 포털 비노출) |

#### 🟢 v3 활성화 — 등급별 6개 필드 (v3.1: `쿠폰 수량` 3개 삭제됨)
