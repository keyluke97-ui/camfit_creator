    ...
    return (
        <div>
            <div className="w-full h-1 bg-subtle rounded-full overflow-hidden">
                <div style={{ width: `${percentage}%` }} className="h-full bg-brand rounded-full transition-all" />
            </div>
            <p className="text-xs text-ink2 mt-1.5">
                {label ? `${label} · ` : ''}{totalCount}명 모집 · 잔여 <span className="text-ink font-semibold">{availableCount}명</span>
            </p>
        </div>
    );
}
