...
export default function ContentRequirements({ channelTypes, hostInstagram, detailUrl, isCouponEvent }: ContentRequirementsProps) {
    ...
    return (
        <div className="bg-subtle p-4 rounded-lg space-y-3">
            <p className="text-sm font-bold text-brand-strong flex items-center gap-1"><BrandIcon name="memo" size={16} />콘텐츠 제작 시 필수 사항</p>
            {channelTypes?.includes('인스타') && (
                <div className="flex items-start gap-2 text-sm text-ink">
                    <span className="text-brand-strong mt-0.5 shrink-0">•</span>
                    <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <p>콘텐츠에 <strong className="text-ink">@camfit_official</strong> 태그 필수</p>
                    </div>
                </div>
            )}
            {channelTypes?.includes('인스타') && hostInstagram && (
                <div className="flex items-start gap-2 text-sm text-ink">
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <p>캠핑장 인스타 <strong className="text-ink">@{hostInstagram}</strong> 태그</p>
                    </div>
                </div>
            )}
                ...
                <div className="w-full">
                    ...
                    {followerLinks.map((link) => (
                        <a
                            ...
                            className="text-brand-strong underline text-xs mt-1.5 inline-flex items-center gap-1 font-semibold"
                        >
                            <BrandIcon name="link" size={13} />{link.label} 바로가기 →
                        </a>
                    ))}
                </div>
            </div>
